...
void FastISel::recomputeInsertPt() {
  ...
  while (FuncInfo.InsertPt != FuncInfo.MBB->end() && FuncInfo.InsertPt->getOpcode() == TargetOpcode::EH_LABEL)
    ++FuncInfo.InsertPt;
}
...
bool FastISel::SelectNop(const User *I, unsigned ISDOpcode) {
	...
	unsigned ResultReg = FastEmit_(VT.getSimpleVT(), VT.getSimpleVT(), ISDOpcode);
	if (ResultReg == 0) return true;/*false*/
...
bool FastISel::SelectInstruction(const Instruction *I) {
  ...
  if (!isa<CallInst>(I) && !isa<NopInst>(I)) {
    recomputeInsertPt();
