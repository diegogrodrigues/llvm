...
//===----------------------------------------------------------------------===//
//                       NopInst Implementation
//===----------------------------------------------------------------------===//
NopInst::NopInst(const NopInst &NI)
  : Instruction(Type::getVoidTy(NI.getContext()), Instruction::Nop, allocHungoffUses(NI.getNumOperands()), NI.getNumOperands()),
    ReservedSpace(NI.getNumOperands()) {

  //Use *OL = OperandList, *InOL = NI.OperandList;
  //for (unsigned I = 0, E = ReservedSpace; I != E; ++I)
    //OL[I] = InOL[I];

  //setCleanup(NI.isCleanup());
}

NopInst::NopInst(LLVMContext &C, Instruction *InsertBefore)
  : Instruction(Type::getVoidTy(C), Instruction::Nop, 0, 0, InsertBefore) {
  //init(PersonalityFn, 1 + NumReservedValues, NameStr);
}
...
NopInst *NopInst::clone_impl() const {
