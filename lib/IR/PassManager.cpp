...
void printInstructions(Function *f){
	...
	for(Function::iterator bb = f->begin(); bb !=f->end(); bb++){
		for(BasicBlock::iterator inst = bb->begin(); inst != bb->end(); inst++){
			errs() <<"printInstructions - count:"<<count++ <<"  BasicBlock:" << bb->getName() << "  Instruction:" <<inst->getOpcodeName()<< "\n";
		}
	}
...
bool FPPassManager::runOnFunction(Function &F) {
  ...
  for (unsigned Index = 0; Index < getNumContainedPasses(); ++Index) {
	//if(Index !=10 && Index !=21 && Index !=22 && Index !=23 && Index !=24 && Index !=25)
	  //  continue;
    FunctionPass *FP = getContainedPass(Index);
    //errs() << "PassIndex:"<< Index << " - PassName: " << FP->getPassName() << "\n";
