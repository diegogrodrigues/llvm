...
class NopInst : public Instruction {
 ...
 unsigned ReservedSpace;
  NopInst(const NopInst &LP);
public:
  enum ClauseType { Catch, Filter };

  explicit NopInst(LLVMContext &C, Instruction *InsertBefore= 0);

  //explicit NopInst(LLVMContext &C, BasicBlock *InsertAtEnd);
...
protected:
  virtual NopInst *clone_impl() const;
public:

  static NopInst *Create(LLVMContext &C, Instruction *InsertBefore = 0);
  //static NopInst *Create(Type *RetTy, BasicBlock *InsertAtEnd);
  ...
  static inline bool classof(const Instruction *I) {
    return I->getOpcode() == Instruction::Nop;
  }
  static inline bool classof(const Value *V) {
    return isa<Instruction>(V) && classof(cast<Instruction>(V));
  }
...
template <>
struct OperandTraits<NopInst> : public HungoffOperandTraits<2> {
};

DEFINE_TRANSPARENT_OPERAND_ACCESSORS(NopInst, Value)
